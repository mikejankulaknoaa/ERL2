...
class Erl2PwmOutput(Erl2Control):
    ...
    def __init__(self,
        ...
        # (check first if this object is an Erl2PwmOutput or a child class)
        if self.__class__.__name__ == 'Erl2PwmOutput':
            self.updateLog()

    def changeHardwareSetting(self):

        # apply the new setting to the output channel
        #print (f"{self.__class__.__name__}: Debug: changing [{self.controlType}] to setting [{int(self.setting)}] on Open-Drain / PWM output channel [{self.__outputPwmChannel}]")
...
def main():
    ...
    root = tk.Tk()
    ttk.Label(root,text='Erl2PwmOutput',font='Arial 30 bold').grid(row=0,column=0)
