...
class Erl2State():
    ...
    def __init__(self, erl2context={}):
        ...
        try:
            ...
            if 'system' not in Erl2Log.logTypes:
                self.__dirName = erl2context['conf']['system']['logDir'] + '/zDebug'
            else:
                self.__dirName = erl2context['conf']['system']['logDir']
    ...
    def get(self, type, name, defaultValue):
        ...
        if type not in self.erl2state:
            self.erl2state[type] = {}

        # add the setting's default value if it doesn't already exist
        if name not in self.erl2state[type]:
            ...
            self.erl2state[type][name] = str(defaultValue)

        # and then parse the saved values to recover floats, lists etc.
        return literal_eval(self.erl2state[type][name])
    ...
    def set(self, type, name, newValue):
        ...
        #print (f"{self.__class__.__name__}: Debug: set({type},{name}) called: old [{str(self.erl2state[type][name])}], new [{str(newValue)}]")

        # check if this setting is new, or exists and is changing
        if name not in self.erl2state[type] or str(newValue) != self.erl2state[type][name]:

            #print (f"{self.__class__.__name__}: Debug: set({type},{name},{newValue}) change detected!")

            # save the updated value to memory as a string
            self.erl2state[type][name] = str(newValue)
